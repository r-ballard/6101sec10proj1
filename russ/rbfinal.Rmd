...
<<<<<<< Updated upstream
<<<<<<< Updated upstream
product_sample <- sample(2, nrow(products), replace=TRUE, prob=c(0.8, 0.2))
=======
...
```{r reviewproductcount}

#TODO delete section
#Check number of reviews by product id
table(reviews$ProductID)
```
...
productreview_training <- (left_join(review_training,
                           product_training %>% select(ProductID,
                                                 HAS_CARAMEL,
                                                 HAS_CHOCOLATE,
                                                 HAS_FRUIT,
                                                 HAS_VANILLA,
...
ingred_test <- preprocess_mca(ingred_test)
=======
>>>>>>> Stashed changes
=======
>>>>>>> Stashed changes
```
There is a known issue with FactoMineR predict.MCA() which causes it to create matrices which are not calcuable for cross-products


```{r ingredient matrix}

#https://towardsdatascience.com/cooking-recipes-with-pca-5c33a4acb7db
...
#factominer issue: https://github.com/husson/FactoMineR/issues/15
...
<<<<<<< Updated upstream
<<<<<<< Updated upstream
#ingredindicator$ProductID <- products$ProductID
...
=======
>>>>>>> Stashed changes
#Can use training productnames instead..
#ingredindicator$ProductID <- products$ProductID
ingredindicator$traintest <- product_sample


#Here we create an aggregate table with indicator sums by train and test for each ingredient. We use this to identify non-contributing factors in either train or test. This was done in order to allow for cross products using FactoRMineR.
<<<<<<< Updated upstream
>>>>>>> Stashed changes
=======
>>>>>>> Stashed changes
ingredcounts <- aggregate(.~traintest,ingredindicator,sum)
ingrednozeros <- ingredcounts %>% select(where(~ all(. != 0)))
...
#TODO - ELIMINATE 0 FIELDS FOR PREDICTION MODEL
#summary(ingred_training)
#which(colSums(ingred_training==0) == nrow(ingred_training))
#which(colSums(ingred_test==0) == nrow(ingred_test))

which(names(ingredindicator) %in% ingrednozeros)
...
<<<<<<< Updated upstream
<<<<<<< Updated upstream
mca1 = MCA(ingred_training, graph = FALSE)

newtest1 <- predict.MCA(object = mca1,
            newdata = ingred_test)

mca1_vars_df = data.frame(mca1$var$coord, Variable = rep(names(train.cats), train.cats))

# data frame with observation coordinates
mca1_obs_df = data.frame(mca1$ind$coord)

mcaout <- predict.MCA(object = mca1,
            newdata = ingred_test)



#not worthwhile, TODO: delete
...
productreview_training <- left_join(productreview_training,
                                   ingred_training.mcadim %>% select('Dim 1',
                                                                     'Dim 2',
                                                                     'Dim 3',
                                                                     'Dim 4',
                                                                     'Dim 5',
...
lda.fit=MASS::lda(Stars∼Dim_1+Dim_2+Dim_3+Dim_4+Dim_5,data=productreview_training)
#subset=train


lda.fit=lda(Direction∼Lag1+Lag2 ,data=Smarket ,subset=train)

ingred_test.mcadim <- as.data.frame(ingred_predict)
ingred_test.mcadim['ProductID'] <- product_test['ProductID']
ingred_test.mcadim['avg_ratings']<-product_training.labels
...
>>>>>>> Stashed changes
```{r producthistograms}
breycol <- brandcolordf[brandcolordf$brand=='breyers','color']
breyfill <- brandcolordf[brandcolordf$brand=='breyers','fill']


ggplot(data = productreview, aes (x = Stars, color=Brand, fill = Brand)) + 
  geom_histogram(binwidth=1,boundary=-.5) +
  labs(title="Distribution of Star Ratings \nFor Icecream Brand: Breyers",
   x="Star Rating",
   y="Review Count\n(in Thousands)") +
  scale_y_continuous(labels = unit_format(unit = "", scale = 1e-3)) +
  scale_fill_manual(values=breyfill) +
  scale_colour_manual(values=breycol) +
...
```{r breyerproductratings}
#Color and fill are inverted here for readability

#sortorder
ratingorder <- products[order(products$Rating, decreasing = TRUE),"ProductID"]


ggplot(products, aes(x=ProductID, y=Rating)) +
  #geom_point(col=paste(breyfill), fill=paste(breycol), alpha=0.7) +
  geom_point(aes(col=HAS_CHOCOLATE), alpha=0.7) +
  labs(x="ProductID", y='Rating') +
  labs(title="Average Ratings for Each Individual Breyers products") + 
  theme(axis.text.x = element_text(angle=90, hjust=1, size = 5)) +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_x_discrete(limits=ratingorder) +
  scale_color_manual(values = c("grey","brown"))


ggplot(products, aes(x=ProductID, y=Rating)) +
  #geom_point(col=paste(breyfill), fill=paste(breycol), alpha=0.7) +
  geom_point(aes(col=HAS_CARAMEL)) +
  labs(x="ProductID", y='Rating') +
  labs(title="Average Ratings for Each Individual Breyers products") + 
  theme(axis.text.x = element_text(angle=90, hjust=1, size = 5)) +
  theme(plot.title = element_text(hjust = 0.5)) +
...
#TODO DELETE, unnecessary with Breyers filtering above
library(ggplot2)
