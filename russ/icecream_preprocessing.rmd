...
prodcombdf$HAS_VANILLA <- as.factor(as.integer(grepl(pattern = "VANILLA", x = prodcombdf$ingredients)))
prodcombdf$HAS_ORGANIC <- as.factor(as.integer(grepl(pattern = "ORGANIC", x = prodcombdf$ingredients)))
prodcombdf$HAS_CARAMEL <- as.factor(as.integer(grepl(pattern = "CARAMEL", x = prodcombdf$ingredients)))

fruitvec <- c("RASPBERR","CHERR","BLUEBERR","BANANA")
prodcombdf$HAS_FRUIT <- as.factor(as.integer(grepl(pattern=paste(fruitvec,collapse="|"), x=prodcombdf$ingredients)))

chocvec <- c("CHOCOLAT","COCO")
prodcombdf$HAS_CHOCOLATE <- as.factor(as.integer(grepl(pattern=paste(chocvec,collapse="|"), x=prodcombdf$ingredients)))

prodcombdf$IS_BAR <- as.factor(as.integer(grepl(pattern = "BAR", x = prodcombdf$name) | 
...
ggplot(reviewdf,aes(Season,stars)) +
  ...
  scale_fill_manual(values=brandcolordf$fill) +
  scale_colour_manual(values=brandcolordf$color) +
  labs(title="Distribution of Product Average Star Ratings \nFor Icecream Brand In A Season",
       x="Season",
       y="Brand Season Average Star Rating") +
  theme(plot.title = element_text(hjust = 0.5))
...
```{r chocolateboxplot}
ggplot(prodcombdf,aes(HAS_CHOCOLATE,rating)) +
  geom_boxplot(aes(fill=HAS_CHOCOLATE)) + 
  scale_fill_manual(values=c("grey","chocolate4"))

ggplot(prodcombdf,aes(HAS_FRUIT,rating)) +
  geom_boxplot(aes(fill=HAS_FRUIT)) + 
  scale_fill_manual(values=c("grey","deeppink"))

ggplot(prodcombdf,aes(HAS_ORGANIC,rating)) +
  geom_boxplot(aes(fill=HAS_ORGANIC)) + 
  scale_fill_manual(values=c("grey","chartreuse3"))

ggplot(prodcombdf,aes(HAS_CARAMEL,rating)) +
  geom_boxplot(aes(fill=HAS_CARAMEL)) + 
  scale_fill_manual(values=c("grey","darkgoldenrod4"))


#only a single bar identified using current method
# ggplot(prodcombdf,aes(IS_BAR,rating)) +
#   geom_boxplot(aes(fill=IS_BAR)) + 
#   scale_fill_manual(values=c("grey","darksalmon"))
