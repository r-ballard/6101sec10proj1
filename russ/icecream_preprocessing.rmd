...
ggplot(data = productreview, aes (x = stars, fill = brand)) + 
  ...
  facet_grid(brand ~ .) +
  scale_fill_manual(values=c("yellow3","green4","brown","pink4")) +
...
ggplot(monthlyratingavg, aes(ym, avgrating, group = brand, color = brand)) + 
  geom_line() +
  labs(title="Monthly Average Star Rating \nFor Icecream Brand",
   x="Year-Month",
   y="Average Rating") + 
  scale_color_manual(values=c("yellow3","green4","brown","pink4")) +
  theme(plot.title = element_text(hjust = 0.5))
```
