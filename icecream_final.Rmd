...
reviews_bkp <- reviews
```
We rename columns in the data for reader interpretability elsewhere. This can sometimes cause issues with joins or when attempting to reference raw data so caution must be used. 
...
In this section we parse the `reviews`, `products`, and joined `productreview` data into training and testing data for subsequent Multiple Correspondant and Linear Discriminant analyses.
...
In this section we conduct Multiple Correspondence Analysis on the Breyers icecream data, specifically engineering features from a one-hot encoded matrix of ingredient contents for the icecream. With review Star rating as the response variable. Here we attempt to answer our research question, "Can we identify ingredients which correlate with Ratings?"
...
```
With our Linear Discriminant Analysis we are able to develop a predictive model with an `Accuracy` of `r cm$overall['Accuracy']` when applied to the `product-ingredient` test set. The model also has an `Accuracy P-Value` of `cm$overall['AccuracyPValue']`, indicating the model is not significant. Reviewing the confusion matrix generated by the `caret` `confusionMatrix` call we see that the uneven factor weights earlier highlighted come in to play in the trained model predictions. In cases besides reviews rated `5` the model is unfortunately insensitive. One avenue of further research for eliminating this bias would be removing reviews with `5` ratings and studying only ratings 1-4, or other subsetting/stratification methods to correct for bias in the `reviews` data.
...
```{r prodrating_shapiro, results='markup'}
...
```{r ratingvariatonmeasures, echo=FALSE}
...
```{r breyers_highrate, echo=FALSE, results='markup'}
...
```{r breyers_lowrate, echo=FALSE, results='markup'}
...
```{r breyers_centraltendency, echo=FALSE}
...
```{r brandratingboxplot, results='markup'}
...
```{r ratingbranddensity, results='markup'}
...
```{r subset-ratingingredients, results='markup'}
...
We see that there are no products across all 4 brands that have organic ingredients. 2 products that are a bar, 1 product that has caramel, and 1 product that has fruit. Then many products that have Chocolate and Vanilla.
...
```{r pred-chocovan, results='markup'}
new_data <- data.frame(
  Brand = "breyers",
...
```{r pred-chococara, results='markup'}
new_data <- data.frame(
  Brand = "breyers",
...
```{r pred-chocoorg, results='markup'}
new_data <- data.frame(
  Brand = "breyers",
...
```{r pred-highrated, results='markup'}
new_data <- data.frame(
  Brand = "breyers",
  Name = "Organic Bar with Chocolate and Fruit",
  HAS_CARAMEL = 0,
  HAS_CHOCOLATE = 1,
  HAS_FRUIT = 1,
  HAS_VANILLA = 0,
...
```{r breyer-chocosubset, results='markup'}
...
```{r breyer-fruitsubset, results='markup'}
...
```{r organic-subset, results='markup'}
...
```{r productsofinterest, results='markup'}
...
Atkinson, W. (2023, April 28). Charting fields and spaces quantitatively: From multiple correspondence analysis to categorical principal components analysis - quality & quantity. SpringerLink. https://link.springer.com/article/10.1007/s11135-023-01669-w 

Sanchez, G. (2012, October 13). 5 functions to do Multiple Correspondence Analysis in R. Visually Enforced. May 1, 2024, https://www.gastonsanchez.com/visually-enforced/how-to/2012/10/13/MCA-in-R/ 

Khoong, W. H. (2023, June 15). Dimensionality Reduction Techniques For Categorical & Continuous Data. Medium. May 1, 2024, https://medium.com/codex/dimensionality-reduction-techniques-for-categorical-continuous-data-75d2bca53100

kassambara. (2017, September 24). MCA - Multiple Correspondence Analysis in R: Essentials. May 1, 2024, http://www.sthda.com/english/articles/31-principal-component-methods-in-r-practical-guide/114-mca-multiple-correspondence-analysis-in-r-essentials/ 
