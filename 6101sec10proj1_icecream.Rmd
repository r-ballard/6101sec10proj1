...
```{r contributiontables, results = 'markup'}
choc_contrib <- 100*caramel_chisq$residuals^2/caramel_chisq$statistic
xkabledply(round(choc_contrib, 3))

fruit_contrib <- 100*fruit_chisq$residuals^2/fruit_chisq$statistic
xkabledply(round(fruit_contrib, 3))

caramel_contrib <- 100*caramel_chisq$residuals^2/caramel_chisq$statistic
xkabledply(round(caramel_contrib, 3))
```



```{r anova}
anovachoc <- aov(Stars ~ HAS_CHOCOLATE, data=productreview)
anovachocsummary <- summary(anovachoc)
tukeyanovachocsummary <- TukeyHSD(anovachoc)

anovafruit <- aov(Stars ~ HAS_FRUIT, data=productreview)
anovafruitsummary <- summary(anovafruit)
tukeyanovafruitsummary <- TukeyHSD(anovafruit)

anovaorg <- aov(Stars ~ HAS_ORGANIC, data=productreview)
anovaorgsummary <- summary(anovaorg)
tukeyanovaorgsummary <- TukeyHSD(anovaorg)

anovacarm <- aov(Stars ~ HAS_CARAMEL, data=productreview)
anovacarmsummary <- summary(anovacarm)
tukeyanovacarmsummary <- TukeyHSD(anovacarm)


tukeyanovacarmsummary$HAS_CARAMEL
tukeyanovacarmsummary$HAS_CARAMEL[1]
```


```{r ingredientcrosstables , results='markup'}
xkabledply(chisq.test(choc_contingency_table)$expected, title = "Cross table for the expected frequencies between Star Rating vs Has Chocolate")

xkabledply(chisq.test(fruit_contingency_table)$expected, title = "Cross table for the expected frequencies between Star Rating vs Has Fruit")

xkabledply(chisq.test(organic_contingency_table)$expected, title = "Cross table for the expected frequencies between Star Rating vs Has Organic")

xkabledply(chisq.test(caramel_contingency_table)$expected, title = "Cross table for the expected frequencies between Star Rating vs Has Caramel")
